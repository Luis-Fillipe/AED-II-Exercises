...
class Hash {
	...
	public void inserir(Filme elemento) {
		...
		//System.out.println("Nome: " + elemento.getTituloOriginal() + " posição " + ascii);
		if (array[ascii] == null) {
	...
	public void pesquisar(String nome) {
		...
		if (array[ascii] != null && array[ascii].getTituloOriginal().compareTo(nome) == 0) {
			...
			ascii++;
			if (ascii < array.length &&array[ascii] != null && array[ascii].getTituloOriginal().compareTo(nome) == 0) {
			contador++;
			resp = true;
			System.out.println("Posicao: " + ascii);
			}
	...
	public static void main(String[] args) throws ParseException, Exception {
		...
		do {
			...
			if (isFim(arquivo) == true) {
				...
				hash.pesquisar(arquivo);
			}
		} while (isFim(arquivo) == false);
		// hash.mostrar();
		try {

			BufferedWriter bw = new BufferedWriter(new FileWriter("753192_hashRehash.txt"));
