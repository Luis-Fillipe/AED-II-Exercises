...
class Arvore {
	...
	private No inserir(Filme elemento, No i) throws Exception {
		if (i == null) {
			...
			if (i.elemento.getTituloOriginal().compareTo(elemento.getTituloOriginal()) < 0) {
				++contador;
	...
	public static void main(String[] args) throws ParseException, Exception {
		...
		for (int i = 0; i < tamanho; i++) {
			...
			if (arquivo.charAt(0) == 'I' && arquivo.charAt(1) == ' ') {
				...
				arvore.remover(clearTag(arquivo, 2));
			}
		}
		...
		try {
			...
			BufferedWriter bw = new BufferedWriter(new FileWriter("753192_arvoreBinaria.txt"));
