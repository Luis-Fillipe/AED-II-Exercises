...
class Lista {
	...
	public boolean pesquisar(String nome) {
		...
		for (Celula i = primeiro.prox; i != null; i = i.prox) {
			if (i.elemento.getTituloOriginal().compareTo(nome) == 0) {
				retorno = true;
				i = ultimo;
			}
		}
...
class Hash {
	...
	boolean pesquisar(String elemento) {
		int pos = key(elemento);
		System.out.println("=> " + elemento);
		boolean resp = array[pos].pesquisar(elemento);
		if (resp == true) {
...
public class TP07 {
	...
	public static void main(String[] args) throws ParseException, Exception {
		...
		try {
			...
			BufferedWriter bw = new BufferedWriter(new FileWriter("753192_hashIndireta.txt"));
