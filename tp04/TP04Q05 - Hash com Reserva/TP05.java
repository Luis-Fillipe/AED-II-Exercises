...
class Hash {
	...
	public Hash(int x) {
		this.array = new Filme[x];
		reserva = 21;
		contador = 0;
	}

	public void inserir(Filme elemento) {
		int ascii = key(elemento.getTituloOriginal());
		if (array[ascii] == null) {
			array[ascii] = elemento.clone();
			contador++;
		} else if (reserva < array.length && array[reserva] == null) {
			array[reserva] = elemento.clone();
			contador++;
			// System.out.println("Tô na reserva meu patrão, camisa "+reserva);
			reserva++;
		} else {
			//System.out.println("Nome: "+elemento.getTituloOriginal()+" key: "+ascii);
		}
	}

	private int key(String nome) {
		int valor = 0;
	...
	public boolean pesquisar(String nome) {
		int ascii = key(nome);
		boolean resp = false;
		if (array[ascii] != null && array[ascii].getTituloOriginal().compareTo(nome) == 0) {
			contador++;
			resp = true;
			System.out.println("Posicao: "+ascii);
		} else {
			for (int i = 21; i < reserva; i++) {
				if (array[i].getTituloOriginal().compareTo(nome) == 0) {
		...
		//System.out.println("NA SAIDA = "+resp);
		return resp;
	...
	public void mostrar(){
		for (int i = 0; i < array.length; i++) {
			if (array[i] != null) {
				System.out.println(array[i].getTituloOriginal()+" KEY= "+i);
			}
	...
	public static void main(String[] args) throws ParseException, Exception {
		...
		do {
			...
			if (isFim(arquivo) == true) {
				...
				boolean resp = hash.pesquisar(arquivo);
				if (resp == false) {
		...
		try {
			...
			BufferedWriter bw = new BufferedWriter(new FileWriter("753192_hashReserva.txt"));
			double tempoFinal = (System.currentTimeMillis() - tempoInicial) / 1000.0;
			bw.append("753192" + "\t" + tempoFinal + "\t" + hash.getContador());
