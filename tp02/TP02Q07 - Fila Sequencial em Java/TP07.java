...
class Fila {
	...
	public void inserir(Filme movie) throws Exception {
		if (((ultimo + 1) % 6) == primeiro) {
			...
			this.remover();
			this.array[ultimo] = movie.clone();
			ultimo = (ultimo + 1) % 6;
		} else {
			array[ultimo] = movie.clone();
	...
	public Filme remover() throws Exception {
		...
		movie = this.array[primeiro].clone();
	...
	public float mediaTempo() {
		...
		//mostrar();
		while (i != this.ultimo) {
			media = media + array[i].getDuracao();
			i = (i + 1) % 6;
		}
		return media / 5;
	}
	...
	public void mostrar() {
		...
		while (i != this.ultimo) {
			System.out.println("["+j+"] "+printString(array[i]));
			i = (i + 1) % 6;
			j++;
		}
