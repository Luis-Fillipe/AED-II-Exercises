...
void selecao(Movie *filmes, int i, int tamanho)
{

    int menor = i;
    for (int j = i; j < tamanho; j++)
    {
        if ((strcmp(filmes[j].original_title, filmes[menor].original_title)) < 0)
        {
            menor = j;
        }
    }
