...
class Fila {
	...
	public Fila(int tamanho) {
		array = new Celula[tamanho];
		primeiro = 0;
		ultimo = 0;
	}

	public void inserir(Filme movie) throws Exception {
		Celula tmp = new Celula(movie);
	...
	public String remover() throws Exception {
		...
		String resp = array[primeiro].elemento.getNome();
	...
	public float mediaTempo() {
		...
		//mostrar();
		while (i != this.ultimo) {
			media = media + array[i].elemento.getDuracao();
			i = (i + 1) % 6;
			j++;
		}
		return media / j;
	...
	public String printString(Filme movie) {
		String saida = "";
		SimpleDateFormat padrao = new SimpleDateFormat("dd/MM/yyyy");
		saida += movie.getNome() + " " + movie.getTituloOriginal() + " "
				+ padrao.format(movie.getData()) + " " + movie.getDuracao()
				+ " " + movie.getGenero() + " " + movie.getIdioma() + " "
				+ movie.getSituacao() + " " + movie.getOrcamento() + " "
				+ movie.imprimirKeywords();
		return saida;
	}
	...
	public void mostrar() {
		...
		while (i != this.ultimo) {
			System.out.println("["+j+"] "+printString(array[i].elemento));
			i = (i + 1) % 6;
			j++;
		}
	...
	public static void main(String[] args) throws ParseException, Exception {
		...
		do {
			...
			if (isFim(arquivo) == true) {
				...
				i++;
				if (i <= 5) {
					mediaTempo = (mediaTempo + movie.getDuracao());
					System.out.println(Math.round(mediaTempo / i));
				} else {
					
					System.out.println(Math.round(filmes.mediaTempo()));
				}
		...
		for (int j = 0; j < tamanho; j++) {
			arquivo = MyIO.readLine(); // ler a linha da string
			if (arquivo.charAt(0) == 'I') {
				Filme movie = new Filme();
				arquivo = clearTag(arquivo, 2);
				movie.searchOnFile(arquivo);
				filmes.inserir(movie);
				System.out.println(Math.round(filmes.mediaTempo()));
				
			} else if (arquivo.charAt(0) == 'R') {
