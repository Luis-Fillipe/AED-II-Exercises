...
public class TP03 {
	...
	public static void swap(Filme[] array, int i, int menor) {
		Filme change = array[i].clone();
		array[i] = array[menor].clone();
		array[menor] = change.clone();
	}

	public static void insertion(Filme[] filmes, int tamanho, Filme movie) {
		for (int i = 1; i <= tamanho; i++) {
			Filme tmp = filmes[i].clone();
			int j = i - 1;
			while((j >= 0)  && (filmes[j].getData().compareTo(tmp.getData()) > 0)){
				filmes[j+1] = filmes[j].clone();
				j--;
			}
			filmes[j+1] = tmp.clone();
		}
	}
