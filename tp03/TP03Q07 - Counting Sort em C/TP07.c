...
void swap(int i, int menor, Movie *array)
{
    Movie change = array[i];
    array[i] = array[menor];
    array[menor] = change;
}

int getMaior(Movie *array, int n) {
    int maior = array[0].duration;

    for (int i = 0; i < n; i++) {
        if(maior < array[i].duration){
            maior = array[i].duration;
        }
    }
    return maior;
}

void sort(Movie * array, int size) {
		for (int i = size-1; i > 0; i--) {
			for (int j = 0; j < i; j++) {
				if (array[j].duration > array[j + 1].duration) {
					swap(j, j + 1, array);
				}
				else if(array[j].duration == array[j+1].duration){
					if (strcmp(array[j].name,"Homem-Aranha: Sem Volta Para Casa") == 0) {
...
void countingsort(Movie *array, int n) {
    ...
    for (int i = 0; i < n; count[array[i].duration]++, i++);

    //Agora, o count[i] contem o numero de elemento menores ou iguais a i
    for(int i = 1; i < tamCount; count[i] += count[i-1], i++);

    //Ordenando
    for(int i = n-1; i >= 0; ordenado[count[array[i].duration]-1] = array[i], count[array[i].duration]--, i--);
...
int main()
{
    ...
    // printf("\n\nopa\n\n");
    countingsort(movies, count_movies);
    sort(movies, count_movies);
    for (int i = 0; i < count_movies; i++)
    {
        movie_print(&movies[i]);
    }
