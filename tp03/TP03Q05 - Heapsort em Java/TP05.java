...
public class TP05 {
	...
	static void constroi(Filme[] array, int tamHeap) {
		for (int i = tamHeap; i > 1 && array[i].getGenero().compareTo(array[i / 2].getGenero()) > 0; i /= 2) {
			swap(array, i, i / 2);
		}
	}

	static void reconstroi(Filme[] array, int tamHeap) {
		int i = 1, filho;
		while (i <= (tamHeap / 2)) {
			if (array[2 * i].getGenero().compareTo(array[2 * i + 1].getGenero()) > 0 || 2 * i == tamHeap) {
				filho = 2 * i;
			} else {
				filho = 2 * i + 1;
			}
	 ...
	 */
	static Filme[] heapsort(Filme[] array, int n) {
	...
	static void swap(Filme[] array, int i, int j) {
		Filme temp = array[i].clone();
		array[i] = array[j].clone();
		array[j] = temp.clone();
	}

	static Filme[] insertion(Filme[] array, int size) {
		int menor, inicio = 0;

		for (int i = 1; i < size; i++) {
			if (array[i].getGenero().compareTo(array[i+1].getGenero()) == 0) {
				inicio = i;
				i = size;
			}
		}
		...
		for (int i = inicio; i < size - 1; i++) {
			...
			for (int j = i + 1; j < size; j++) {
				if (array[j].getGenero().compareTo(array[menor].getGenero()) == 0) {
	...
	public static void main(String[] args) throws ParseException, Exception {
		...
		filmes = heapsort(filmes, i);
		filmes = insertion(filmes, i);
