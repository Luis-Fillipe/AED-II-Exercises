...
class merge {
	 ...
	 */
	public void merge(Filme[] filmes, int quantidade) {
	...
	public void intercalar(int esq, int meio, int dir) {
		...
		for (i = 0; i < n1; i++) {
			left[i] = array[esq + i].clone();
		}

		// Inicializar segundo subarray
		for (j = 0; j < n2; j++) {
			right[j] = array[meio + j + 1].clone();
		}
		j = 0;
		i = 0;
		k = esq;
		while (i < n1 && j < n2) {
			if (left[i].getOrcamento() < right[j].getOrcamento()) {
	...
	public void insertion() {
		...
		for (int i = inicio; i < size - 1; i++) {
			...
			for (int j = i + 1; j < size; j++) {
				if (array[j].getOrcamento() == array[menor].getOrcamento()) {
...
public class TP09 {
	...
	public static void main(String[] args) throws ParseException, Exception {
		...
		array.merge(filmes, i);
		array.mergesort(0, i - 1);
		array.insertion();
		array.print();
