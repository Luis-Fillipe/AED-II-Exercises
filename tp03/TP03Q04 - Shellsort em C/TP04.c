...
void shellsort(Movie *array, int size)
{
    int i, j;
    int h = 1;
    ...
    while (h > 0)
    {
        for (i = h; i < size; i++)
        {
            char* value = array[i].original_language;
            j = i;
            while (j > h - 1 && strcmp(value, array[j-h].original_language) < 0)
            {
