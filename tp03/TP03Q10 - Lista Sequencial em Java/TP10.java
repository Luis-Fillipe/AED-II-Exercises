...
class Lista {
	...
	public void mostrar() {
		...
		for (int j = 0; i != null; j++, i = i.prox) {
			System.out.println("["+j+"] "+ i.elemento.print());
		}
	}
	...
	public static void main(String[] args) throws ParseException, Exception {
		...
		for (int j = 0; j < tamanho; j++) {
			...
			if (arquivo.charAt(0) == 'I' && arquivo.charAt(1) == 'I') {
				...
				movie.searchOnFile(arquivo);
				lista.inserirFim(movie);
				//funciona mas erro ao mostrar
			} else if (arquivo.charAt(0) == 'R' && arquivo.charAt(1) == 'I') {
				String nome = lista.removerInicio();
				System.out.println("(R) " + nome);
				//funciona
			} else if (arquivo.charAt(0) == 'R' && arquivo.charAt(1) == 'F') {
				String nome = lista.removerFim();
				System.out.println("(R) " + nome);
				//funciona
			} else if (arquivo.charAt(0) == 'R' && arquivo.charAt(1) == '*') {
				int posicao = Integer.parseInt(clearTag(arquivo, 3));
				String nome = lista.remover(posicao);
				System.out.println("(R) " + nome);
				//funciona
			} else if (arquivo.charAt(0) == 'I' && arquivo.charAt(1) == '*') {
				arquivo = clearTag(arquivo, 3);
				String pos = "";
				pos = pos + arquivo.charAt(0) + arquivo.charAt(1);
				arquivo = clearTag(arquivo, 3);
				Filme movie = new Filme();
				movie.searchOnFile(arquivo);
